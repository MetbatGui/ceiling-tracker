...
- **일일 자동 추적** (`daily-update`): GDrive에서 대상 연도 DB를 받아 로컬을 최신화하고,
  당일 상한가 종목을 KRX에서 수집(휴장일 사이 공백은 자동 백필)한 뒤, 엑셀 리포트 렌더링과
  산출물/DB Drive 업로드까지 한 번에 끝냅니다. 컨테이너 내장 cron이 평일 15:55 KST에
  자동 실행합니다.
- **기간/연도 백필** (`range-update`, `annual-update`): 임의 기간 또는 연도 범위를 한 번에
  수집합니다(대상 연도 DB를 먼저 GDrive에서 받아 덮어쓴 뒤 수집만 하며, 리포트 렌더링은
  하지 않습니다).
- **엑셀 리포트 수동 재생성** (`export-excel`): SQLite에 쌓인 데이터를 임의 연도/기간
  단위로 다시 조회해 엑셀로 렌더링합니다(`--drive`로 구글 드라이브 업로드까지). 과거 특정
  구간만 다시 뽑고 싶을 때 쓰는 수동 도구이며, 데일리 자동 파이프라인은 `daily-update`
  하나로 끝나므로 이 커맨드를 별도로 실행할 필요가 없습니다.
...
│   ├── application/      # DailyRoutineService, RangeRoutineService, ReportPublishService,
│   │                     # ExcelExportService 등
│   ├── infrastructure/   # KrxDirectStockInfoAdapter, SqliteCohortRepository, db_sync.py,
...
- **CLI는 조립·호출만, 오케스트레이션 서비스가 DB 연결부터 업로드까지 전체 흐름을
  소유**합니다(`orchestration_guide.md` §1). `db_sync.py`(다운로드/업로드 순수 함수)와
  `ReportPublishService`(export+업로드 조합)는 CLI가 아니라 `DailyRoutineService`/
  `RangeRoutineService`가 직접 호출합니다.
- `DailyRoutineService`가 (Drive 사용 시) 대상 연도 DB 다운로드 → 휴장일 판정
  (`CalendarService`) → 수집(`DailyUpdateService`) + 갭 백필 → `ReportPublishService`로
  엑셀 렌더링 + 산출물/DB 업로드까지 한 실행 흐름으로 소유합니다.
- `RangeRoutineService`는 같은 다운로드 세션 모델을 쓰지만 리포트 렌더링은 하지 않습니다
  (수동 백필 도구 - 리포트가 필요하면 `export-excel`을 별도로 실행).
- `SqliteCohortRepository`가 SSOT이며, `db/{year}.db`로 연도별 분리 저장합니다.
- 저장소는 `LocalStorageAdapter`/`GoogleDriveAdapter` 두 구현체가 있고, Drive에 저장할 때는
  항상 로컬에도 자동 백업합니다(`ReportPublishService`/`export-excel --drive` 둘 다 동일).
- DB 업로드 대상은 리포트가 조회한 날짜 범위가 아니라 **로컬에 실제 존재하는 `db/*.db` 파일
  ...
  (`db_sync.py`의 `upload_db_files` 참고, `orchestration_guide.md` §3 원칙).
- 다운로드는 GDrive에 파일이 있는데 읽기가 실패하면 "없음"과 구분해 세션을 중단하고
  (`db_ssot_guide.md` §6.1), 로컬에 이미 파일이 있어도 항상 다시 받아 덮어씁니다(§6.2).
...
# 엑셀 리포트 수동 재생성 (로컬) - 데일리 자동 파이프라인은 daily-update가 알아서 함
uv run python src/cli.py export-excel --year 2026

# 엑셀 리포트 수동 재생성 + 구글 드라이브 업로드 (+ db/*.db 백업 업로드)
...
컨테이너 내장 cron이 스케줄에 따라 `daily-update` 하나로 데일리 파이프라인(수집 + 리포트
생성 + 업로드)을 자동 실행합니다. 스케줄은 `docker/crontab`을 참고하세요(기본: 평일
15:55 KST — 장마감 이후 시세 확정 대기).
...
2. **`daily-update`가 다운로드~업로드 전체 흐름을 소유**: `run-daily.sh`는 이제
   `daily-update` 한 커맨드만 호출합니다 — 예전엔 `export-excel`을 셸 스크립트가 별도
   2단계로 이어붙였지만, 그러면 수동 재실행 등 셸 스크립트를 거치지 않는 경로에서
   리포트 생성이 누락될 수 있었습니다. 새 CLI 커맨드를 추가할 때도 예외를 삼키고
   조용히 exit 0으로 끝나지 않도록 주의하세요(`docker_guide.md` §10).
3. **Drive 저장 시 로컬 백업이 자동으로 따라붙음**: `ReportPublishService`(daily-update 경로)와
   `export-excel --drive` 둘 다, Drive 저장 성공 여부와 무관하게 로컬에도 항상 저장합니다 —
   순수 로컬 저장소로 전환할 필요 없이 항상 로컬에 최신 산출물이 남습니다.
